...
function response(room, msg, sender, isGroupChat, replier, ImageDB) {
    if(isGroupChat==false&&sender.startsWith("DSHS")){
      if(sender.startsWith("DSHS")&&sender.length!=14) replier.reply("방 나가기를 눌러 나가주시고 다시 1대1채팅을 들어와주세요.\n들어오실 때 닉네임을 DSHS+복잡한숫자10자리로 해주세요.\nEx) DSHS0123456789 (띄어쓰기없음)"); //닉네임이 올바르지 않음   
      else{
      if(msg.startsWith("/신규등록")){
         
        if(getUserIDRegistered(msg.split(" ")[1]))replier.reply("이미 등록된 아이디입니다.");
        else{
          var ID=msg.split(" ")[1].replace(/(|)/g,"");
          var PW=msg.split(" ")[2].replace(/(|)/g,"");
          var token=doLogin([ID,PW]);
          var name=getName(token);
          if(name=="") replier.reply("로그인을 실패하였습니다. 정확하게 입력했는지 확인해주세요.");
          else{
      ...
      try{
        if(!getUserNickRegistered(sender)&&(msg.startsWith("/자습")||msg.startsWith("/주말")||msg.startsWith("/리셋")||msg.startsWith("/벌점")||msg.startsWith("/관리자"))){
          replier.reply("학생현황관리시스템 /신규등록을 먼저 진행해주세요.😠");
          }
      else{
        if(msg=="/dev"){
          var token=getToken(sender);
          var resultraw= Jsoup.connect("https://djs.weschool.kr/sph_manager/weekend/index.php?mode=layer_apply&kind=sunday&stamp=1622905200&order_no=1&chk_no=6&chk_week=1&pid=pid5ac350c1951e3")
    ...
    .post()
    .toString();
    replier.reply(resultraw);
    resultraw=resultraw.split("<th>");
    var res="";
    var res2="";
    for(var i=9;i<resultraw.length;i++){
      res+=" ("+(i-8)+") "+resultraw[i].split("</th>")[0]+"\\n";
      res2+="\""+resultraw[i].split("id=\"")[1].split("\"")[0]+"\",";
    }
    replier.reply(res);
    replier.reply(res2);
        }
    ...
    if(number.startsWith("2021")) var iinfo="《교실 목록》\n(1) 독서실\n(2) 부모외출\n(3) 병외출\n(4) 종교외출\n(5) 기타외출\n(6) R&E연구외출\n(7) 외박\n(8) 기숙사 호실\n(9) 다산관 대강당\n(10) [일신4층]독서실\n(11) [일신2층]1-1교실\n(12) [일신2층]1-2교실\n(13) [일신2층]1-3교실\n(14) [일신2층]1-4교실\n(15) [일신2층]1-5교실\n(16) [일신2층]1-6교실\n(17) [일신1층]시청각실\n(18) [다산7층](707)천문학실험실\n(19) [다산7층](708)천체동아리실\n(20) [다산7층](710)대기과학실험실\n(21) [다산7층](705)무한상상실\n(22) [다산6층](607)화학실험실\n(23) [다산6층](608)화학실험실\n(24) [다산4층](412)물리소규모실험실\n(25) [다산3층](302)일반물리실험실\n(26) [다산2층](202)박테리아배양실\n(27) [다산2층](202)동물세포배양실\n(28) [다산2층](203)일반생명과학실험실\n(29) [다산2층](206)현미경실험실\n(30) [다산2층](207)생물실험실\n(31) [다산2층](209)전자현미경실\n(32) [다산1층](103)일반화학실험실\n(33) [다산1층](104)무기화학실험실\n(34) [다산1층](108)화학기기분석실\n(35) [탐의2층]중회의실\n(36) [다산6층](606)융합실험실\n(37) [일신2층]다목적강의실2(4반, 5반, 6반)\n(38) [일신2층]다목적강의실1(1반, 2반, 3반)\n";
      ...
      {
        if(!getSelfStudyTime()) replier.reply("⛔ 자습신청 시간이 아닙니다.\n(신청시간: 08:00~18:30)");
        else{
    ...
    if(msg.substr(6).indexOf("a")!=-1||msg.substr(6).indexOf("A")!=-1){
      ...
      if(msg.indexOf("토")!=-1){
        var weekind="saturday";
         var n=0;
         var str="( * 마감 시까지 신청이 완료되지 않을 경우 \"독서실\"로 자동 설정됩니다.)";
      }
     ...
     var weekind="sunday";
      ...
      result=result.split(str)[1].split("<th>");
      ...
      for(var i=0;i<winfo[n].pid.length;i++)
      {
    ...
    if(msg.substr(6).indexOf("a")!=-1||msg.substr(6).indexOf("A")!=-1){
     ...
     var command=msg.substr(6).split("/");
      ...
      for(id=0;id<command.length;id++){
        if(command[id]=="") command[id]=precommand[id];
      }
      }
      else if(command2[1]!=undefined&&command[1]==undefined){
        if(command2[1]=="토"){
   ...
   if(number.startsWith("2019")){
     sinfoa=" (1)오전자율   09:00 ~ 12:00{1}\n (2)오후자율1 13:00 ~ 14:40{2}\n (3)오후자율2 14:50 ~ 16:20{3}\n (4)오후자율3 16:30 ~ 18:00{4}\n (5)야간자율1 19:30 ~ 21:00{5}\n (6)야간자율2 21:30 ~ 22:40{6}\n (7)야간자율3 22:40 ~ 23:40{7}";
      ...
      if(dobeban[sender]!=undefined&&wtime-dobeban[sender]>3600000){
        ...
        replier.reply("b방장봇방",sender+"가 관리자 호출. "+dobe[sender]);
        replier.reply("관리자를 호출하였습니다. 무분별한 호출 행위 발생 시 호출이 제한됩니다.");
      }
      ...
      else if(wtime-dobe["Timer"+sender]<300000){
        if(dobe[sender]>=3){
          dobeban[sender]=wtime;
          dobe[sender]=0;
          delete dobe["Timer"+sender];
          replier.reply("b방장봇방",sender+"가 관리자 존나 불러서 막았어요. "+dobe[sender]);
          replier.reply("도배가 감지되어 관리자 호출이 제한됩니다.");
        }
        else{
          dobe[sender]+=1;
          replier.reply("b방장봇방",sender+"가 관리자 호출. "+dobe[sender]);
   ...
   else if(msg.startsWith("혹시 액션이 어떻게 되는지 아세요")){
       replier.reply("무야호~");
    }
    else{
      replier.markAsRead();
     ...
     // replier.reply("b전체방","자동자습신청 시작!");
      ...
      //replier.reply("b전체방","✅자동자습신청 완료!");
    }
    else if(room=="b방장봇방"&&msg=="/자동주말자습신청"){
     ...
     // replier.reply("b전체방","자동주말자습신청 시작!");
      ...
      var cnt1=0;
      senders=["DSHS0509108754","DSHS1294857510","DSHS6894662811"];
      ...
      for(var j=0;j<DB.length;j++){
        ...
        if(DB[j]["flag"][1]){
          ...
          replier.reply("자동주말자습신청 토\n"+DB[j].nameasWeekendSat+"\n"+DB[j].User);
      ...
      for(var j=0;j<DB.length;j++){
          ...
          for(var i=7;i<result.length;i++)
        {
          ...
          replier.reply("자동주말자습신청 일\n"+DB[j].nameasWeekendSun+"\n"+DB[j].User);
          cnt2+=1;
          for(var i=0;i<apply.length;i++){
            if(apply[i+1]=="") continue;
      ...
      replier.reply("b방장봇방","자동주말자습신청 끝. 인원: "+cnt1+"\n"+cnt2);
      //replier.reply("b전체방","✅자동자습신청 완료!");
    }
}
...
function doPointCheck(username){ //벌점을 확인함.
    ...
    pListRaw[1]=pListRaw[1].slice(0,-(pTotal.length)-2);
...
function doSelfStudyCheckNum(username){
  ...
  for(var n=0;n<3;n++){
    ...
    for(var i=2;i<result.length;i++){
      ...
      pnames.push(String(result[i-1].substring(result[i-1].indexOf(")")+2,result[i-1].length)));
      pnumbertot.push(temp[1]);
    }
    var res="(1) 독서실: (지정석)\n";
    for (var i=0;i<pnames.length;i++){
      res+="("+(i+2)+") "+pnames[i]+": ("+pnumbers[0][i]+","+pnumbers[1][i]+","+pnumbers[2][i]+"/"+pnumbertot[i]+")\n";
    }
    return res;
...
function doWeekendNameConvert(arr,number,n,preset){
   ...
   for(var i=1;i<arr.length;i++){
     if(arr[i]==""||arr[i]==undefined||arr[i]=="0"){
       if(arr[i]=="0"||preset[i]==undefined) result.push("(설정 안됨)");
