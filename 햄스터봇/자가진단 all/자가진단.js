  ...
  if(yoill1=="토"||yoill2=="일"){
    java.lang.Thread.sleep(3200000);
       ...
       replier.reply("b방장봇방","자가진단 시작! (총 "+pnum+"명)");
        ...
        if(interval[i]==undefined) interval[i]=5000;
        ...
        var ss=Date.now ();
        Log.i (interval[i])
        interval [i]=Number(interval[i])-ss+fs;
        if (interval [i]<=0) interval [i]=3000;
          cnt+=1;
          Log.e (ss-fs)
          Log.i(interval[i]+"\n"+cnt+"번째 완료\n"+sub.message);
          java.lang.Thread.sleep(interval[i]);
        }
        
       replier.reply("b전체방","✅자가진단 완료 (에러: "+errcnt+"명)" + "\u200b".repeat(500) + "\n\n" + res);
